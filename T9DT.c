...
void traverseTrie(Node *tree, char *input)
{
    ...
    int traverser = 0;
    // to check later on if user enters # or not
    char inputChecker = '0';
    while (tree != NULL && input[0] != '\0')
    {
        inputChecker = input[0];
    ...
    {
        // If trying to find more synonyms
        if (inputChecker == '#')
        {
            printf("There are no more T9 synonyms.\n");
        }
        else
        {
            // If trying to find a new word.
